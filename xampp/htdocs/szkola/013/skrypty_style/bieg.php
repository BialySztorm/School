...
<body>
    ...
    try {
        ...
        $dane = @$connection->query("SELECT MIN(wyniki.wynik) FROM wyniki WHERE wyniki.dyscyplina_id = $dyscyplina");
        $curr_line = $dane->fetch_assoc();
        $best = $curr_line['MIN(wyniki.wynik)'];
        if ($uczestnicy)
            echo "<div class='record'>Nasz rekord: " . $best . " s</div>";
        else
