...
function randNum() {
    tmp = getJSON("data.json")["digits"]
    curr = Math.round(Math.random() * (tmp.length - 1))
    // console.log(curr)
    question.innerHTML = capitalizeFirstLetter(tmp[curr])
    tmp1 = [curr]
    for (i = 0; i < answers.length - 1; i++) {
        tmp2 = Math.round(Math.random() * (tmp.length - 1))
        check = 0
